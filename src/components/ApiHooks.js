...
const ApiHooks = () => {
  ...
  function userProfile(term) {
      ...
      .catch(function (error) {
        if (error.response) {
          ...
          console.log(error.response.status);
          console.log(error.response.headers);
          setDataAvailable("User does not exist or there was an issue authenticating");
  ...
  //This function uses the Profile Data that got fetched and
  //if the array is not empty it will display the design
  ...
  return(
    ...
    <div className="form-search ">
      {/* The onChange event watches and stores the input field */}
      ...
      {/* Button that triggers the showResults function */}
